...
try {
  ...
  await until(`document.body.textContent.includes('Data yüklənməyib')`);
  assert(await evaluate(`document.body.textContent.includes('LIVE BACKEND')`));
  await clickText('Hadisə jurnalı yüklə');
  ...
  await until(`document.body.textContent.includes('Yükləmə tamamlandı')`);
  await clickText('Qrafa qayıt');
  ...
  await clickText('Ödəniş anı görünüşü');
  assert(await evaluate(`document.body.textContent.includes('7,607')`));
  await clickText('Ödəniş anı görünüşü');
  await key('Enter');
  await until(`document.body.textContent.includes('Model interpretasiyası')`);
  const bounds = await evaluate(`(() => {const label=Array.from(document.querySelectorAll('span')).find(e=>e.textContent==='Model interpretasiyası'); const panel=label.parentElement.parentElement.parentElement; const r=panel.getBoundingClientRect(); return {x:r.x,y:r.y,right:r.right,bottom:r.bottom,width:r.width};})()`);
  ...
  assert(!panelText.includes('320 gün'), 'No unrelated whale explanation');
  assert(!panelText.includes('**'), 'No bold Markdown markers');
  assert(panelText.includes('Claude tərəfindən yazılıb') || panelText.includes('Şablon izah') || panelText.includes('İzah gecikdi'));
  console.log('Panel visible; explanation prefetched:', bounds, panelText.includes('Claude tərəfindən yazılıb') ? 'Claude' : 'fallback');
  await key('f'); await key('Enter');
  await until(`document.body.textContent.includes('Fırıldaq kimi qeydə alındı') && document.body.textContent.includes('1/1')`);
  ...
  await until(`document.body.textContent.includes('2 / 3 halqa')`);
  await clickText('Ödəniş anı görünüşü');
  ...
  await clickText('Qərarlanmış'); await key('Enter');
  await until(`document.body.textContent.includes('İzah gecikdi')`);
  assert(!await evaluate(`document.body.textContent.includes('Claude tərəfindən yazılıb') || document.body.textContent.includes('320 gün')`));
  await key('r'); await key('Enter');
  await until(`document.body.textContent.includes('Qərar saxlanmadı')`);
